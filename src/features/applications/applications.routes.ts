...
const CreateBodySchema = z.object({
  ...
  status: z.enum(["applied", "screening", "interview", "offer", "hired", "rejected", "withdrawn"] as const).optional(),
...
// ═══════════════════════════════════════════════════════
// VIEW: GET /applications/views/pipeline
...
app.get('/views/pipeline', async (c) => {
  ...
  const viewFields = ["id","jobId","candidateId","status","appliedAt"];
  ...
  const searchableColumns = ["status","notes"];
  if (params.search && searchableColumns.length > 0) {
    const searchTerm = `%${params.search}%`;
    const searchConditions = searchableColumns
      .filter((col: string) => col in applications)
      .map((col: string) => like(applications[col as keyof typeof applications] as any, searchTerm));
  ...
  for (const field of viewFields) {
    if (field in applications) {
      selectFields[field] = applications[field as keyof typeof applications];
    }
  }

  // Build and execute query
  let query = db.select(selectFields).from(applications);
...
const parseResult = await parseJsonWithSchema(c.req.raw, BatchCreateBodySchema);
  ...
  if (validRecords.length > 0) {
    try {
      success = await db.insert(applications).values(validRecords).returning();
    } catch (error: any) {
...
app.post('/:id/advance', async (c) => {
  ...
  const [record] = await rawDb.select().from(applications)
    .where(and(buildFirewallConditions(ctx), eq(applications.id, id)));
  ...
  if (accessConfig && !await evaluateAccess(accessConfig, ctx, record)) {
    
    return c.json(AccessErrors.roleRequired((accessConfig as any).roles || [], ctx.roles), 403);
  }
  ...
  // Execute action
  try {
    const result = await actions["advance"].execute({ db, rawDb: undefined, ctx, record, input, services, c, auditFields });
...
app.post('/:id/hire', async (c) => {
  ...
  // Execute action
  try {
    const result = await actions["hire"].execute({ db, rawDb: undefined, ctx, record, input, services, c, auditFields });
...
app.post('/:id/reject', async (c) => {
  ...
  // Execute action
  try {
    const result = await actions["reject"].execute({ db, rawDb: undefined, ctx, record, input, services, c, auditFields });
...
app.post('/:id/withdraw', async (c) => {
  ...
  // Execute action
  try {
    const result = await actions["withdraw"].execute({ db, rawDb: undefined, ctx, record, input, services, c, auditFields });
