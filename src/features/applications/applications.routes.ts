...
const FILTER_PARSERS: Record<string, (v: string) => unknown> = {
  "id": (v: string) => v,
  "jobId": (v: string) => v,
  "candidateId": (v: string) => v,
  "status": (v: string) => v,
  "appliedAt": (v: string) => v,
  "notes": (v: string) => v,
...
app.get('/', async (c) => {
  ...
  if (isMultiSort) {
    for (const part of sortParam.split(',')) {
      ...
      const col = applications[fieldName as keyof typeof applications] as any;
    ...
    if (acceptSort(sortParam)) {
      const col = applications[sortParam as keyof typeof applications] as any;
      if (typeof col === 'object' && col) {
  ...
  // Build & execute query.
  let query = selectFields ? db.select(selectFields).from(applications) : db.select().from(applications);
  ...
  if (params.total === 'true') {
    let countQuery = db.select({ value: count() }).from(applications);
    if (filterConditions.length > 0) {
...
// ═══════════════════════════════════════════════════════
// BATCH CREATE: POST /applications/batch
// ═══════════════════════════════════════════════════════
...
const parseResult = await parseJsonWithSchema(c.req.raw, BatchCreateBodySchema);
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    const data = {
      id: 'app_' + crypto.randomUUID().replace(/-/g, ''),
      ...record,
      ...{},
  ...
  if (validRecords.length > 0) {
    try {
      success = await db.insert(applications).values(validRecords.map((v) => v.data)).returning();
    } catch (error: any) {
...
// ═══════════════════════════════════════════════════════
// BATCH UPDATE: PATCH /applications/batch
// ═══════════════════════════════════════════════════════
app.patch('/batch', async (c) => {
  ...
  const existingRecords = await db.select().from(applications)
    .where(and(buildFirewallConditions(ctx), inArray(applications.id, ids)));
  ...
  const processBatch = async (handle: any) => {
    ...
    modifiedAt: new Date().toISOString(),
      ...
      // Execute update
      try {
        const result = await handle.update(applications)
          .set(data)
          .where(and(buildFirewallConditions(ctx), eq(applications.id, record.id)))
          .returning();
...
// ═══════════════════════════════════════════════════════
// BATCH DELETE: DELETE /applications/batch
// ═══════════════════════════════════════════════════════
app.delete('/batch', async (c) => {
  ...
  let success: any[] = [];
    ...
    modifiedAt: new Date().toISOString(),
          ...
          .where(and(buildFirewallConditions(ctx), inArray(applications.id, validIds)))
          .returning();
...
// ADVANCE: POST /applications/:id/advance
app.post('/:id/advance', async (c) => {
  ...
  const accessConfig = (actions["advance"] as any).access;
  if (accessConfig) {
    ...
    if (accessFailure !== null) {
      ...
      if (accessFailure === 'record') {
        return c.json(
          AccessErrors.recordPreconditionFailed('advance', (accessConfig as any).record),
          409
        );
      }
      return c.json(
  ...
  const transitionConfig = (actions["advance"] as any).transition;
  if (transitionConfig) {
...
app.post('/:id/hire', async (c) => {
  ...
  // Access check (role + record preconditions)
  const accessConfig = (actions["hire"] as any).access;
  if (accessConfig) {
    const accessFailure = await evaluateAccessReason(accessConfig, ctx, record);
    if (accessFailure !== null) {
      ...
      if (accessFailure === 'record') {
        return c.json(
          AccessErrors.recordPreconditionFailed('hire', (accessConfig as any).record),
          409
        );
      }
      return c.json(
  ...
  // Transition validator (compiled state machine)
  const transitionConfig = (actions["hire"] as any).transition;
  if (transitionConfig) {
    const currentValue = (record as any)[transitionConfig.field];
    const allowedTargets = transitionConfig.fromTo?.[currentValue] ?? [];
    const targetValue = transitionConfig.to ?? (input as any)?.[transitionConfig.via ?? 'nextStatus'];
...
// REJECT: POST /applications/:id/reject
app.post('/:id/reject', async (c) => {
  ...
  // Access check (role + record preconditions)
  const accessConfig = (actions["reject"] as any).access;
  if (accessConfig) {
    const accessFailure = await evaluateAccessReason(accessConfig, ctx, record);
    if (accessFailure !== null) {
      ...
      if (accessFailure === 'record') {
        return c.json(
          AccessErrors.recordPreconditionFailed('reject', (accessConfig as any).record),
          409
        );
      }
      return c.json(
  ...
  // Transition validator (compiled state machine)
  const transitionConfig = (actions["reject"] as any).transition;
  if (transitionConfig) {
    const currentValue = (record as any)[transitionConfig.field];
    const allowedTargets = transitionConfig.fromTo?.[currentValue] ?? [];
    const targetValue = transitionConfig.to ?? (input as any)?.[transitionConfig.via ?? 'nextStatus'];
...
app.post('/:id/withdraw', async (c) => {
  ...
  // Access check (role + record preconditions)
  const accessConfig = (actions["withdraw"] as any).access;
  if (accessConfig) {
    const accessFailure = await evaluateAccessReason(accessConfig, ctx, record);
    if (accessFailure !== null) {
      ...
      if (accessFailure === 'record') {
        return c.json(
          AccessErrors.recordPreconditionFailed('withdraw', (accessConfig as any).record),
          409
        );
      }
      return c.json(
  ...
  // Transition validator (compiled state machine)
  const transitionConfig = (actions["withdraw"] as any).transition;
  if (transitionConfig) {
    const currentValue = (record as any)[transitionConfig.field];
    const allowedTargets = transitionConfig.fromTo?.[currentValue] ?? [];
    const targetValue = transitionConfig.to ?? (input as any)?.[transitionConfig.via ?? 'nextStatus'];
