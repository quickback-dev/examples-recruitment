...
const CreateBodySchema = z.object({
  ...
  employmentType: z.enum(["full-time", "part-time", "contract", "internship"] as const).optional(),
}).strict();
...
const DEFAULT_SEARCHABLE: string[] = ["headline","summary"];
...
const KNOWN_COLUMNS: ReadonlySet<string> = new Set(["id","jobId","slug","headline","summary","location","employmentType","organizationId"]);
...
app.get('/', async (c) => {
  ...
  if (isMultiSort) {
    for (const part of sortParam.split(',')) {
      ...
      const col = jobPostings[fieldName as keyof typeof jobPostings] as any;
    ...
    if (acceptSort(sortParam)) {
      const col = jobPostings[sortParam as keyof typeof jobPostings] as any;
      if (typeof col === 'object' && col) {
  ...
  if (viewFields && viewFields.length > 0) {
    ...
    for (const f of viewFields) {
      if (KNOWN_COLUMNS.has(f)) projected[f] = jobPostings[f as keyof typeof jobPostings];
    }
    ...
    for (const f of params.fields.split(',')) {
      ...
      if (fieldName && KNOWN_COLUMNS.has(fieldName)) {
        requested[fieldName] = jobPostings[fieldName as keyof typeof jobPostings];
      }
    }
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    const column = jobPostings[field as keyof typeof jobPostings] as any;
  ...
  if (params.search && searchableColumns.length > 0) {
    ...
    const searchConditions = searchableColumns
      ...
      .map((col: string) => sql`${jobPostings[col as keyof typeof jobPostings] as any} LIKE ${searchTerm} ESCAPE '\\'`);
  ...
  // Build & execute query.
  let query = selectFields ? db.select(selectFields).from(jobPostings) : db.select().from(jobPostings);
  ...
  if (params.total === 'true') {
    let countQuery = db.select({ value: count() }).from(jobPostings);
    if (filterConditions.length > 0) {
...
// ═══════════════════════════════════════════════════════
// BATCH CREATE: POST /jobPostings/batch
// ═══════════════════════════════════════════════════════
...
const parseResult = await parseJsonWithSchema(c.req.raw, BatchCreateBodySchema);
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    const data = {
      id: 'jbP_' + crypto.randomUUID().replace(/-/g, ''),
      ...record,
      ...{},
  ...
  if (validRecords.length > 0) {
    try {
      success = await db.insert(jobPostings).values(validRecords.map((v) => v.data)).returning();
    } catch (error: any) {
...
// ═══════════════════════════════════════════════════════
// BATCH UPDATE: PATCH /jobPostings/batch
// ═══════════════════════════════════════════════════════
app.patch('/batch', async (c) => {
  ...
  const existingRecords = await db.select().from(jobPostings)
    .where(and(buildFirewallConditions(ctx), inArray(jobPostings.id, ids)));
  ...
  const processBatch = async (handle: any) => {
    ...
    modifiedAt: new Date().toISOString(),
      ...
      // Execute update
      try {
        const result = await handle.update(jobPostings)
          .set(data)
          .where(and(buildFirewallConditions(ctx), eq(jobPostings.id, record.id)))
          .returning();
...
// ═══════════════════════════════════════════════════════
// BATCH DELETE: DELETE /jobPostings/batch
// ═══════════════════════════════════════════════════════
app.delete('/batch', async (c) => {
  ...
  let success: any[] = [];
    ...
    modifiedAt: new Date().toISOString(),
          ...
          .where(and(buildFirewallConditions(ctx), inArray(jobPostings.id, validIds)))
          .returning();
...
const KNOWN_COLUMNS_GET: ReadonlySet<string> = new Set(["id","jobId","slug","headline","summary","location","employmentType","organizationId"]);
...
app.get('/:id', async (c) => {
  ...
  // Query with firewall
  const baseQuery = selectFields ? db.select(selectFields).from(jobPostings) : db.select().from(jobPostings);
  const [record] = await baseQuery.where(and(buildFirewallConditions(effectiveCtx), eq(jobPostings.id, id)));
...
const parseResult = await parseJsonWithSchema(c.req.raw, CreateBodySchema);
  ...
  try {
    const result = await db.insert(jobPostings).values(data).returning();
...
app.patch('/:id', async (c) => {
  ...
  // Fetch with firewall
  const [record] = await db.select().from(jobPostings)
    .where(and(buildFirewallConditions(effectiveCtx), eq(jobPostings.id, id)));
  ...
  try {
    const result = await db.update(jobPostings).set(data)
      .where(and(buildFirewallConditions(effectiveCtx), eq(jobPostings.id, id))).returning();
